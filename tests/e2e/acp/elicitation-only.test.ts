...
// permissions) still work; fire-and-forget UI calls surface as `[ACP]`
// agent_message_chunk warnings instead of vanishing.
...
describe("ACP integration — elicitation only", () => {
					...
					toolCalls: [
						{
							function: {
								name: "bash",
								arguments: JSON.stringify({ command: "touch /tmp/kimchi-acp-marker-elicit.txt" }),
							},
						},
					],
				},
	...
	it("surfaces fire-and-forget UI calls as agent_message_chunk warnings instead of dropping them", async () => {
		...
		expect(fixture.client.elicitationRequests.length, "elicitation/create call").toBeGreaterThanOrEqual(1)
